...
def newyorker_caption_contest_idefics(args): 
    ...
    for val_inst in nyc_data_five_val:
        ...
        prompts.append([
        f"User: Please describe the image and explain the joke. {train_inst_1['input']}",
        #"User: Please describe this image and explain the joke.",
        train_inst_1['image'],
        "<end_of_utterance>",
        f"\nAssistant:{train_inst_1['target']}.<end_of_utterance>",

        f"User: Please describe the image and explain the joke. {train_inst_2['input']}",
        train_inst_2['image'],
        "<end_of_utterance>",
        f"\nAssistant:{train_inst_2['target']}.<end_of_utterance>",

        f"User: According to the above two examples, please describe the image and explain the joke. {val_inst['input']}",
        val_inst['image'],
        "<end_of_utterance>",
        "\nAssistant:",
        ])
...
def newyorker_caption_contest_llama2(args): 
    ...
    for i, val_inst in enumerate(nyc_data_five_val):         
        ...
        prompt =  f"Please describe the image and explain the joke.{train_inst_1['input']} [/INST] {train_inst_1['target']} </s><s>[INST]\
        Please describe the image and explain the joke. {train_inst_2['input']} [/INST] {train_inst_2['target']} </s><s>[INST]\
        According to the above two examples, please describe the image and explain the joke. {val_inst['input']} [/INST]"         
